...
Take a look at the list to make sure that it looks right and contains all the files you want to crop:

```{r}
csv_files
```

We also need to read in the **l**ookup table of **d**eployment and **r**etrieval **times**, or `ldrtimes`:

```{r}
ldrtimes = readxl::read_xlsx(paste0(ldrtimes_loc, ldrtimes_fn))
```

Next we crop the files!
